...
export const gameTemplates: Game[] = [
  ...
  },
  {
    id: 'game-template-5',
    title: 'ゲームテンプレート 5',
    description: '戦略ゲームのテンプレートです。じっくり考えて戦略を練りましょう。',
    icon: '🎯',
    category: 'Strategy',
  ...
  },
  {
    id: 'game-template-6',
    title: 'ゲームテンプレート 6',
    description: 'スポーツゲームのテンプレートです。体を動かした気分になれます。',
    icon: '⚽',
    category: 'Sport',
...
export const getGameStats = (): GameStats => {
  ...
  const availableGames = gameTemplates.filter(game => game.status === 'Available').length;
  const comingSoonGames = gameTemplates.filter(game => game.status === 'Coming Soon').length;
  
  const gamesByCategory = gameTemplates.reduce((acc, game) => {
    acc[game.category] = (acc[game.category] || 0) + 1;
    return acc;
  }, {} as Record<string, number>);
  
  const gamesByDifficulty = gameTemplates.reduce((acc, game) => {
